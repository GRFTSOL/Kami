...
> This file is the agent instruction file for generic agentic tools (Codex, opencode, pi, etc.).
> Claude Code users: see `SKILL.md` for the same instructions with Claude-specific routing.
...
|---|---|---|---|
| "one-pager / 方案 / 执行摘要 / exec summary" | One-Pager | `assets/one-pager.html` | `assets/one-pager-en.html` |
| "white paper / 白皮书 / 长文 / 年度总结 / technical report" | Long Doc | `assets/long-doc.html` | `assets/long-doc-en.html` |
| "formal letter / 信件 / 辞职信 / 推荐信 / memo" | Letter | `assets/letter.html` | `assets/letter-en.html` |
| "portfolio / 作品集 / case studies" | Portfolio | `assets/portfolio.html` | `assets/portfolio-en.html` |
| "resume / CV / 简历" | Resume | `assets/resume.html` | `assets/resume-en.html` |
| "slides / PPT / deck / 演示" | Slides | `assets/slides.py` | `assets/slides-en.py` |
...
Read `references/diagrams.md` / `diagrams.en.md` before drawing. Extract the `<svg>` block from the template and drop it into a `<figure>` inside long-doc / portfolio.
...
When the user hands over raw material (meeting notes, brain dump, existing doc, chat transcript, scattered points):
...
3. **Gap-check**: list what the template needs but the raw content doesn't have, presented as a compact table
...
| **Layout tweak** | Adjusting spacing, moving sections, changing font size within spec. CSS touched. | `CHEATSHEET.md` + template |
...
| **Diagram** | Embedding SVG in a doc. | `references/diagrams.md` only |

Full spec files:
- Design: `references/design.md` (CN) / `references/design.en.md` (EN)
- Writing: `references/writing.md` / `references/writing.en.md`
- Production: `references/production.md` / `references/production.en.md`
- Diagrams: `references/diagrams.md` / `references/diagrams.en.md`
...
Visual anomalies (tag double rectangle, font fallback, page break issues) -> `references/production.md` Part 4.
...
**English**
- Main serif: Newsreader (Google Fonts, open source) - headlines and body
- Sans: Inter (open source) - UI elements only (labels, eyebrows, meta)
- Fallback: Charter (macOS) / Georgia (cross-platform), Helvetica Neue / system-ui

Font files sit next to HTML files; `@font-face` uses relative paths.
...
- User wants Material / Fluent / Tailwind default style
- Need dark / cyberpunk / futurist aesthetic (this is deliberately editorial)
- Need saturated multi-color (this has one accent)
- Need cartoon / animation / illustration style
